...
🎨 **The open-source Claude Design alternative.** &nbsp;🌐 **Self-hosted browser workspace, with an optional desktop shell.** &nbsp;⚡ **Composable skills, brand-grade `DESIGN.md` design systems, and ready-to-use plugins.** &nbsp;🖼️ Generates **web · desktop · mobile prototypes**, **live dashboards / artifacts**, **decks**, **images**, **video**, plus **HyperFrames** motion graphics. 🔒 Sandboxed iframe preview · HTML / PDF / PPTX / MP4 export. &nbsp;🤖 **Runs on DeepSeek Harness (`dsh`) · Claude Code · OpenClaw · Codex · Cursor · OpenCode · Qwen · Copilot · Amp · Hermes · Kimi · Antigravity and 25 distinct local CLI executables**, or any OpenAI-compatible endpoint via BYOK.
...
- 🌐 **Browser-first, local-first, BYOK at every layer.** The daemon serves the Web UI and API as one self-hostable application; Electron remains an optional shell. Product analytics and session replay are consent-gated; scrubbed safety and reliability telemetry is always on. Before describing daemon data paths, contributors and operators MUST read `AGENTS.md` → **Daemon data directory contract**. This README MUST NOT restate it.
...
| Self-host / browser | ❌ | ❌ | ❌ | **✅ Docker + daemon-served Web UI** |
...
### 🌐 Open the Web app (recommended)

OpenDesign runs as a daemon-served browser application. Use Docker for a
contained BYOK deployment, or run from source when the daemon should detect and
launch coding-agent CLIs installed on the host. The Electron desktop shell is
optional and is no longer part of the default development startup.
